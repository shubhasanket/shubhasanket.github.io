...
This project is a PyTorch implementation of the **Auto-Encoder with Regression (AER)** framework for univariate time-series anomaly detection. AER combines reconstruction-based detection, an auto-encoder trained on sliding windows of the "normal" signal, with a bi-directional regression head on the latent representations, which improves sensitivity to subtle anomalies that a pure reconstruction loss tends to smooth over.
